...
```typescript
// Inside tests/fixtures/example/.forge/modules/test.ts
...
1. **User declares git dependencies** in `.forge/config.yml`:
   ```yaml
   dependencies:
...
**Configuration:**
```json
// .forge/tsconfig.json (generated by forge install)
{
  "compilerOptions": {
    "baseUrl": ".",
    "paths": {
...
- [ ] Can declare git dependencies in `.forge/config.yml`
...
# 3. Use in module
cat > .forge/modules/hello.ts <<EOF
